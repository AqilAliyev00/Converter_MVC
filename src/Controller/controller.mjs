export default class Controller {
  ...
  getAPI() {
    fetch(
      `https://api.exchangerate.host/convert?from=${this.model.leftValie}&to=${this.model.rightValue}`
    )
      .then((response) => response.json())
      .then((data) => {
        this.data = data.result;
        this.view.rateFrom.innerText = `1 ${this.model.leftValie} = ${this.data} ${this.model.rightValue} `;
        this.view.rateIn.innerText = `1 ${this.model.rightValue} = ${
          Math.floor((1 / this.data) * 10 ** 6) / 10 ** 6
        } ${this.model.leftValie}`;
        this.view.inputIn.value = this.data * this.view.inputFrom.value;
      });
  }
}
